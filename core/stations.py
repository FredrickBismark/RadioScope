...
class SomaFMSource(StationSource):
    ...
    def _normalize(self, channels: list[dict]) -> list[Station]:
        ...
        for ch in channels:
            ...
            # Construct a direct ice server URL rather than storing the .pls playlist
            # URL, which libvlc MediaPlayer cannot follow (it parses the playlist but
            # never plays the sub-items).
            playlists = ch.get("playlists", [])
            url = ""
            bitrate = 0
            for pl in playlists:
                if pl.get("format") == "mp3" and pl.get("quality", "") == "highest":
                    bitrate = pl.get("bitrate", 128) or 128
                    url = f"https://ice2.somafm.com/{channel_id}-{bitrate}-mp3"
                    break
            if not url and playlists:
                pl = playlists[0]
                bitrate = pl.get("bitrate", 128) or 128
                fmt = pl.get("format", "mp3")
                url = f"https://ice2.somafm.com/{channel_id}-{bitrate}-{fmt}"
